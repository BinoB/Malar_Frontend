...
export const createOrder = order => async(dispatch) => {
    try {
       ...
       const {data} = await axios.post(`${URL}/api/v1/order/new`, order)
...
export const deleteOrder = id => async(dispatch) => {
    try {
       ...
       await axios.delete(`${URL}/api/v1/admin/order/${id}`)
...
export const updateOrder = (id, orderData)  => async(dispatch) => {
    try {
       ...
       const { data} = await axios.put(`${URL}/api/v1/admin/order/${id}`, orderData)
